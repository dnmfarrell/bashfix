...
die "Error: bashfix.pl can only upgrade versions 3 or 4 of Bash, but $version was found. Exiting ..."
...
die "Your Bash is already fully patched!" unless $bash_patches_count{ $major_minor_version } != $patch_version;
...
sub download_bash
{
    ...
    system "wget -q -O $tmpdir/$bash.tar.gz https://ftp.gnu.org/pub/gnu/bash/$bash.tar.gz";
    system("cd $tmpdir && tar zxf $bash.tar.gz");

    # apply patches
    print "Downloading patches\n";
    my $version_no_dot = substr($version, 0, 1) . substr($version, 2, 1);
    my $num_patches = $bash_patches_count{$short_version} || die "Error getting number of patches for Bash";

    for (1..$num_patches)
    {
        my $patch_number = sprintf "%03d", $_;
        my $patch_url = "https://ftp.gnu.org/pub/gnu/bash/bash-$short_version-patches/bash$version_no_dot-$patch_number";
        system "cd $tmpdir/$bash && curl -s $patch_url | patch -N -p0 &> /dev/null";
    }
...
sub check_prereqs
{
    ...
    for my $bin (@prereq_bins)
    {
        ...
        die "$bin is required for install but not found, install it via your package manager" 
...
sub update_bash
{
    ...
    die "Error cannot find new Bash binary at $new_bash_bin_path" unless -e $new_bash_bin_path;
